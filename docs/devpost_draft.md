...
- **NemoClaw active-learning curator + inspiration feed (the third pillar).** A single NemoClaw agent runs in the uvicorn lifespan during idle time. 5 of every 6 iterations are *active-learning*: the gap-finder identifies the predictor's weakest bin, Gemma translates that gap into `yt-dlp ytsearch20:` queries (no manual URL list ever), the agent scrapes Shorts that fill the gap, refits the ridge in-process, rolls back on R² regression. The 6th iteration harvests trending Shorts into a date-partitioned pool that powers a third UI section on `/library`: *"trending Shorts that match your style"* — creator centroid cosine-ranked against the trending pool, top-3 with outbound links to the originals. The box gets smarter on its own, and answers the third creator question — *what should I make next?* NeMo-stack alignment with the Blackwell silicon underneath the GX10 is the reason for choosing NemoClaw over OpenClaw.
